...
class Player:
  ...
  def update(self, keys, control_map):
    ...
    # 水平移動入力
    left_key = control_map.get('left')
    right_key = control_map.get('right')
    jump_key = control_map.get('jump')

    if left_key is not None and keys[left_key]:
      self.vx -= accel
      self.facing_right = False
    elif right_key is not None and keys[right_key]:
      self.vx += accel
      self.facing_right = True
    else:
      if self.vx > 0:
    ...
    # ジャンプ処理
    if jump_key is not None and keys[jump_key]:
      if self.on_ground:
        # 地上ジャンプ
...
def main():
  ...
  btn_1p_rect = pygame.Rect(center_x - BTN_WIDTH //
                            2, center_y - BTN_HEIGHT, BTN_WIDTH, BTN_HEIGHT)
    ...
    if game_state == STATE_OPENING:
      ...
      if not is_opening_animation_done:
        is_opening_animation_done = (
            time.time() - opening_start_time) >= opening_show_duration

      if (time.time() - opening_start_time) < opening_show_duration:
        # アニメーション中
        ...
        # キー入力待ちのメッセージ
        press_key_text = "Press any key or click to start"
      ...
      if not voice_played_after_loading and "areyouready" in voice_dict:
        try:
          ...
          # get_length may not exist for some sound types; ignore safely
          voice_dict["areyouready"].play()
      ...
      # --- プレイヤー更新 (モードに基づく) ---
      if play_mode == 1 and player1:
        player1.update(keys, control_map_p1)
        highest_y = player1.y
      elif play_mode == 2 and player1 and player2:
        player1.update(keys, control_map_p1)
      ...
      if play_mode == 1 and player1 and camera1:
        ...
        # 分割線を描画
        pygame.draw.line(
            screen, (0, 0, 0), (HALF_SCREEN_WIDTH, 0), (HALF_SCREEN_WIDTH, SCREEN_HEIGHT), 3)
